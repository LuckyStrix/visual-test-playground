...
class _YesNoRunMixin:
    """Shared run_gui wrapper for yes/no Base subclasses.

    Attaches the hit/FA summary even when the run aborts mid-test."""
...
def wait_ms(win, ms):
    ...
    try:
        ...
        raise ValueError(f"wait_ms requires a finite ms, got {ms!r}") from e
    ...
    try:
        ...
        raise RuntimeError("Stimulus window closed") from None
...
def feedback(canvas, cx, cy, ok, streak=0, points=0, win=None):
    ...
    if ok and streak >= 2:
        canvas.create_text(
            cx,
            cy + 48,
            text=f"{streak}x streak  +{points}",
            fill="gold",
            font=("Arial", 16, "bold"),
        )
    elif ok and points:
        canvas.create_text(cx, cy + 48, text=f"+{points}", fill="gold", font=("Arial", 16, "bold"))
    canvas.update()
    try:
        try:
...
class Base:
    ...
    def __init__(self, name, logger, sp, n_trials=40, feedback=True, practice_trials=3, seed=None):
        ...
        self.stair = Staircase(
            start_val=sp["start_val"],
            step_sizes=sp["step_sizes"],
            n_reversals=sp.get("n_reversals", 8),
            n_trials_max=self.n_trials,
            min_val=sp["min_val"],
            max_val=sp["max_val"],
    ...
    def run_gui(self, canvas, win):
        ...
        try:
            while n_stair < self.n_trials and t < max_total:
                ...
                self.logger.log_trial(
                    test=self.name, trial=n_main, level=lvl, correct=ok, rt_s=round(rt, 3), **extra
                )
                if done:
                    break
...
class ContrastDetection2IFC(Base):
    ...
    def __init__(
        ...
        super().__init__(
            name, logger, staircase_params, n_trials, feedback, practice_trials, seed=seed
        )
        self.ppd = ppd
...
class ColorDiscrimination2IFC(Base):
    ...
    def trial(self, canvas, win, lvl, main):
        ...
        canvas.create_text(
            cx,
            cy,
            text="1 = first had different colours   2 = second",
            fill="white",
            font=("Arial", 16),
        )
        win.update()
...
class StaticContrast(_YesNoRunMixin, Base):
...
class StaticColorBullseye(_YesNoRunMixin, Base):
    ...
    def trial(self, canvas, win, lvl, main):
        ...
        canvas.create_text(
            cx, cy + self.px, text="R = center REDDER   B = center BLUER", fill="white"
        )
        win.update()
        ...
        return (
            ...
            rt,
            {
                ...
                "surround": surr_col,
                "center_redder": truth,
                "said_redder": said_redder,
                "catch": catch,
            },
        )


class BrightnessMatch(_YesNoRunMixin, Base):
    ...
    def trial(self, canvas, win, lvl, main):
        ...
        canvas.create_text(
            cx, cy + self.px * 0.62, text="Y = same grey   N = different", fill="white"
        )
        win.update()
        r, rt = get_key(win, ["y", "Y", "n", "N"])
        said_same = r.lower() == "y"
...
class VernierJudgment(Base):
    ...
    def trial(self, canvas, win, lvl, main):
        ...
        canvas.create_text(
            cx, cy + self.px * 0.62, text="Left / Right arrow: lower bar?", fill="white"
        )
        win.update()
...
class MaskedGabor(_YesNoRunMixin, Base):
    ...
    def trial(self, canvas, win, lvl, main):
        ...
        canvas.delete("all")
        canvas.create_text(
            cx, cy, text="Y = stripes   N = nothing", fill="white", font=("Arial", 20)
        )
        win.update()
        r, rt = get_key(win, ["y", "Y", "n", "N"])
        said = r.lower() == "y"
        ...
        try:
            for _t in range(self.n_trials):
                while True:
                    ...
                    def pre(e, _early=early, _quit=quit_):
                        if e.keysym == "Escape":
                            _quit.append(1)
                        else:
                            _early.append(1)
